...
const MainForecast = ({
  ...
  const handleAddToFavorites = () => {
    ...
    cogoToast.success(
      `Successfully saved ${cityCode.LocalizedName} to favorites`
    );
  };

  const handleDeleteFromFavorites = () => {
    const existingFav = favorites.find(
      favorite => favorite.id === cityCode.AdministrativeArea.ID
    );
    setIsFavorite(!isFavorite);
    dispatch(deleteFavorite(existingFav));
    cogoToast.warn(
      `Successfully deleted ${cityCode.LocalizedName} from favorites`
    );
  };

  const FavoriteIcon = () => {
    const existingFav = favorites.findIndex(
      favorite => favorite.cityName === cityCode.LocalizedName
    );
    if (existingFav !== -1) {
      return (
          ...
          <div className="card">
            <div className="header">
              ...
              <div className="center">
                <img
                  src={`https://apidev.accuweather.com/developers/Media/Default/WeatherIcons/${cityWeather.WeatherIcon}-s.png`}
                  alt=""
                />
                <h1 className="value">
                  {isFahrenheit
                  ...
                  {isFahrenheit ? <span>&deg;F</span> : <span>&#176;C</span>}
                </h1>
            ...
            <div className={`${darkMode ? "dark-mode" : ""} details`}>
              <h1 className="title">
                {cityCode.LocalizedName}, {cityCode.Country.LocalizedName}{" "}
              </h1>
